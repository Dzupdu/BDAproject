...
parameters {
 ...
 model {
    ...
    amu0 ~ normal(50,100); // we have observations at x = 1 so lets just use something close to that
...
parameters {
 ...
 model {
   ...
   for ( j in 1: Nc ){
     for ( i in 1: N ){
        y[j, i] ~ normal(alpha[j] - beta[j] * pow(lambda[j], x1[i] - Amp[j]*normal_lpdf(i | mu_dip[j], sigma_dip[j]) ) , sigma);
     }
   }
 }
 ...
 generated quantities {
    ...
    for ( j in 1: Nc ){
      for(i in (N+1):(N+Npred)){
        ypred[j,i - N] = normal_rng(alpha[j] - beta[j] * pow(lambda[j], i)- Amp[j]*normal_lpdf(i | mu_dip[j], sigma_dip[j]) , sigma);
      }
    }
    
    for ( j in 1: Nc ){
      for ( i in 1: N){
        log_like[(j-1)*N + i] = normal_lpdf(y[j, i] | (alpha[j] - beta[j] * pow(lambda[j], i)) - Amp[j]*normal_lpdf(i | mu_dip[j], sigma_dip[j]) , sigma);
        y_rep[(j-1)*N + i] = normal_rng(alpha[j] - beta[j] * pow(lambda[j], i)- Amp[j]*normal_lpdf(i | mu_dip[j], sigma_dip[j]), sigma);
      }
    }
}
"
...
continents_powerlaw_wdip <- rstan::sampling(sm_powerlaw_wdip, data = data_continents, control = list(max_treedepth = 15)) 
...
```
```{r}
loo_compare(loo_cubic, loo_power,loo_linear)
```

```{r}
par(mfrow=c(1,3))
plot(loo_power, main = "Powerlaw fit Pareto-k")
plot(loo_cubic, main = "cubic fit Pareto-k")
plot(loo_linear, main = "linear fit Pareto-k")
```
